...
import { createPropMaterials, type PropSlot, type SlotMaterial } from './materials.ts'
...
export interface BuildContext<C, S extends PropSlot> {
 ...
 */
const MOTTLE_BY_SLOT: Readonly<Record<PropSlot, number>> = {
  /*
   * Lower across the board than the medieval kit's, and that is the point.
   *
   * Those materials were worked by hand and every one of them varied: a stave
   * differs from the stave beside it, straw is never twice the same. These are
   * manufactured. An injection moulding is the same colour everywhere it is
   * the same part, and mottling it is what makes a new plastic bin look like a
   * salvaged one.
   *
   * So the high values here are the exceptions, and each is a surface that
   * genuinely varies at the scale you see it: grain in timber, weave in
   * fabric, aggregate in concrete, and the spangle that hot-dip zinc actually
   * has and that nothing else in the kit does.
   */
  wood: 1,
  fabric: 1.1,
  concrete: 0.9,
  ...
  stainless: 0.2,
  plastic: 0.18,
  retroreflective: 0.15,
  ceramic: 0.12,
  glass: 0.1,
  glassTinted: 0.1,
  plasticGloss: 0.1,
  chrome: 0.08,
  // The vertex colour IS the result on an unlit material, so any variation
  // here is variation in the light itself rather than in the surface.
  emissive: 0,
}
...
>(
  ...
  const defaults = createPropMaterials(scope, options.slots)
  ...
  function build(): void {
    ...
    const geometriesOf = (part: BuiltPart<S>): BufferGeometry[] =>
      [part, ...(part.extras ?? [])]
        // An emitter neither TAKES shadow nor CASTS one. `emissive` is an
        // unlit material, so its vertex colour is the final colour going
        // straight to the screen, and darkening it with occlusion is the same
        // as switching the light off. The rule is tied to the slot itself
        // rather than to a per-model flag, so it cannot be forgotten on the
        // one object that needed it.
        .filter((body) => body.slot !== 'emissive')
    ...
    for (const [name, part] of Object.entries(produced) as Array<[P, BuiltPart<S> | undefined]>) {
      ...
      for (const body of [part, ...(part.extras ?? [])]) {
        ...
        mesh.userData.vibe3d = {
          model: `@contemporary-props/${options.id}`,
          part: name,
